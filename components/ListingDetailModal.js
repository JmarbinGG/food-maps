    ...
    try {
      ...
      if (window.databaseService && window.databaseService.updateListing) {
        ...
        if (!resp || !resp.success) {
          if (typeof window.showAlert === 'function') 
            window.showAlert(resp?.error || 'Failed to save changes.', { title: 'Error', variant: 'error' });
          return;
        }
      ...
      if (typeof window.showAlert === 'function') 
        window.showAlert('Failed to save changes.', { title: 'Error', variant: 'error' });
    }
  };
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {user && canFavoriteListing && (
              ...
              >
                <svg className={`w-6 h-6 ${isFavorite ? 'text-yellow-700' : 'text-gray-500'}`} viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <div className="p-6 space-y-6">
          ...
          {isEditing && canEdit ? (
            ...
            <div className="space-y-6">
              ...
              {/* Food Information */}
              <div>
                <h4 className="text-sm font-semibold text-gray-900 mb-3">Food Information</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-xs text-gray-600 mb-1">Category</div>
                    <div className="text-sm font-medium text-gray-900 capitalize">{listing.category || '—'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-600 mb-1">Quantity</div>
                    <div className="text-sm font-medium text-gray-900">{listing.qty} {listing.unit}</div>
                  </div>
                  {listing.weight && (
                    <div>
                      <div className="text-xs text-gray-600 mb-1">Weight</div>
                      <div className="text-sm font-medium text-gray-900">{listing.weight} lbs</div>
                    </div>
                  )}
                  {listing.perishability && (
                    <div>
                      <div className="text-xs text-gray-600 mb-1">Perishability</div>
                      <div className="text-sm font-medium text-gray-900 capitalize">{listing.perishability}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Pickup Information */}
              <div>
                <h4 className="text-sm font-semibold text-gray-900 mb-3">Pickup Information</h4>
                <div className="space-y-3">
                  <div>
                    <div className="text-xs text-gray-600 mb-1">Pickup Address</div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {listing.pickup_window_start && (
                        <div>
                          ...
                          <div className="text-sm font-medium text-gray-900">
                            {new Date(listing.pickup_window_start).toLocaleString('en-US', {
                              month: 'short',
                              day: 'numeric',
                              hour: 'numeric',
                              minute: '2-digit'
                      ...
                      {listing.pickup_window_end && (
                        <div>
                          ...
                          <div className="text-sm font-medium text-gray-900">
                            {new Date(listing.pickup_window_end).toLocaleString('en-US', {
                              month: 'short',
                              day: 'numeric',
                              hour: 'numeric',
                              minute: '2-digit'
                  ...
                  {(listing.coords_lat && listing.coords_lng) && (
                    <div>
                      <button
              ...
              {/* Listing Information */}
              <div>
                <h4 className="text-sm font-semibold text-gray-900 mb-3">Listing Information</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
