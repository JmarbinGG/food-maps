...
@pytest.mark.asyncio
async def test_non_digit_user_id_rejected():
    """UUID / non-numeric user ids are not supported on the MySQL route path."""
    r = await execute_tool(
        "show_route_to_listing",
        {
            "user_id": "56e3c110-8e22-4756-b98e-02d2d5c81a36",
            "listing_id": "42",
        },
    )
    assert "error" in r
    assert "user_id" in r["error"].lower()
