...
def _build_system_prompt(training_data: dict) -> str:
    ...
    action_policy = (
        ...
        "\n"
        "### POST-LISTING DRAFT/CONFIRM FLOW (CRITICAL)\n"
        "When the user asks to share/donate/post food, you MUST use the "
        "two-step flow:\n"
        "  1. First call post_food_listing WITHOUT confirmed=true. The "
        "tool returns `draft: true`, `needs_confirmation: true`, a "
        "`draft_listing` object, and a `corrections` list of changes the "
        "server made automatically (past expiration, mismatched perishability, "
        "missing pickup window, etc.).\n"
        "  2. In your reply, clearly tell the user: 'Here is the draft I'd "
        "like to post', then list every field of draft_listing in plain "
        "language (title, qty + unit, category, perishability, pickup window, "
        "expiration, address). If `corrections` is non-empty, EMPHASIZE each "
        "correction with a leading '⚠ Adjusted:' so the user sees what you "
        "fixed (e.g. '⚠ Adjusted expiration to 2026-04-30 because the date "
        "you gave was already past').\n"
        "  3. End with a clear question: 'Should I post this listing as is, "
        "or change anything (e.g. expiration, pickup time, address)?' Wait "
        "for the user to reply.\n"
        "  4. Only after the user explicitly approves ('yes', 'post it', "
        "'looks good', 'go ahead') call post_food_listing AGAIN with the "
        "same fields plus confirmed=true (apply any tweaks the user asked "
        "for). After that call succeeds, announce 'Listing posted!' with the "
        "title and listing id.\n"
        "Never write a listing on the first call. Never hide a correction "
        "from the user — they need to know what you changed."
    )
    return (
