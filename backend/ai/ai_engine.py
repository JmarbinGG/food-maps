...
def _build_system_prompt(training_data: dict) -> str:
    ...
    action_policy = (
        ...
        "\n"
        "### POST-AS-SOON-AS-POSSIBLE RULE (CRITICAL — DO NOT VIOLATE)\n"
        "The ONLY truly required fields for post_food_listing are:\n"
        "  • title (what the food is, e.g. 'sourdough bread')\n"
        "  • qty (how many)\n"
        "  • address (the donor's profile address counts — you do NOT "
        "have to ask if their profile already has one)\n"
        "Everything else (category, perishability, expiration_date, "
        "pickup_window, unit, allergens, photo) is OPTIONAL and the "
        "server fills smart defaults. ONCE YOU HAVE title + qty (and "
        "an address from profile or message), CALL post_food_listing "
        "IMMEDIATELY. Do NOT ask for a photo, expiration date, pickup "
        "window, allergens, or 'anything else'. Post first, then offer "
        "to add extras AFTER ('Posted! Want to add a photo or special "
        "pickup hours?').\n"
        "\n"
        "Minimum-viable dialog (the model MUST emulate this brevity):\n"
        "  Donor: 'I have 3 loaves of sourdough bread to donate'\n"
        "  AI:    <profile has address — calls post_food_listing(title="
        "'Sourdough bread', qty=3) immediately>\n"
        "         'Posted! 3 loaves of sourdough are live, listing #42. "
        "Want to add a photo or pickup hours?'\n"
        "\n"
        ...
        "ask for it ONCE, then post.\n"
        "\n"
        "Hard rules for this flow:\n"
        "  1. NEVER ask more than two clarifying questions before "
        "calling post_food_listing. If you've asked twice and still "
        "don't have title+qty+address, post with whatever you have or "
        "explain what's missing.\n"
        "  2. NEVER ask about: perishability, category, expiration "
        "date, pickup window times, unit, dietary tags, allergens, "
        "or 'is there anything else'. Infer or skip.\n"
        "  3. NEVER ask 'pickup or drop-off?' — every listing is "
        "pickup at the donor's address by default.\n"
        "  4. NEVER ask the donor to confirm their profile address "
        "before posting. Just use it. If it's wrong they'll tell you.\n"
        "  5. PHOTO IS OPTIONAL. Do not ask for one before posting. "
        "After posting, you may offer 'Want to add a photo?' as a "
        "follow-up.\n"
        "  6. PARSE FREE TEXT FIRST. If the donor writes a full "
        "sentence with multiple facts ('I have 3 sourdough loaves'), "
        "extract title+qty in one shot and post immediately.\n"
        "  7. ACKNOWLEDGE warmly but BRIEFLY ('Got it', 'Posted!'). "
        "No long preambles.\n"
        "  8. SAME PATTERN for post_food_request — minimum-viable "
        "fields, call the tool fast, offer extras after.\n"
        "\n"
        "### BULK UPLOAD (CSV / PDF)\n"
